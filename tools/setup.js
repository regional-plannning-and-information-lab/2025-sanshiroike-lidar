...
export const downloadXyzFiles = async (overwrite) => {
	...
	const downloadRepo = "https://github.com/regional-plannning-and-information-lab/2025-sanshiroike-lidar/releases/download/v1.0.0/";
	const files = [
...
export const convertXyzToLas = async (overwrite) => {
	...
	files.forEach(async file => {
		...
		const pipelineFilePath = path.join(pdalPipelineDir, `${fileNameWithoutExt}-pipeline.json`);
		if (!fs.existsSync(pipelineFilePath) || overwrite) {
			const pipelineContent = {
				"pipeline": [
					...
					{
						"type": "writers.las",
						"filename": path.join(__dirname, "raw-las", `${fileNameWithoutExt}.las`),
						"scale_x": 0.001,
						"scale_y": 0.001,
						"scale_z": 0.001,
						"offset_x": "auto",
	...
	files.forEach(file => {
		const fileNameWithoutExt = path.basename(file, path.extname(file));
		const pipelineFilePath = path.join(pdalPipelineDir, `${fileNameWithoutExt}-pipeline.json`);
		const outputLasFilePath = path.join(rawLasDir, `${fileNameWithoutExt}.las`);
		...
		if (fs.existsSync(pipelineFilePath)) {
			...
			pdalProcess.on("close", code => {
				if (code === 0) {
					console.log(`PDAL processing completed for ${fileNameWithoutExt}.las`);
				} else {
